"""Port of coordination/TestTrajectoryEnvelopeScheduling.java from the
meta-csp-tutorial repo (M23).

The Java original draws the geometries/constraint network and opens a
``TrajectoryEnvelopeAnimator`` (``JTSDrawingPanel``, ``ConstraintNetwork
.draw``, ``utility/UI`` -- Swing, not ported, see D10); dropped here,
keeping the refinement/scheduling/printing logic. Paths resolved against
this repo's ``tests/data/paths/`` fixtures, per the precedent set in M22's
...
def main() -> None:
    ...
    for i in range(_NUM_ROBOTS):
        ...
        assert driving.trajectory is not None
        ...
        release = AllenIntervalConstraint(
            AllenIntervalConstraint.Type.Release, Bounds(_CURRENT_TIME, _CURRENT_TIME)
        )
